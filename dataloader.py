...
def load_images(folder, batch_size, k, n_workers):
	...
	train_data = DataLoader(train_dataset, num_workers=n_workers, pin_memory=True, 
		batch_sampler=BatchSampler(ImagesSampler(train_dataset, k, shuffle=True), batch_size=batch_size, drop_last=False))

	valid_data = DataLoader(valid_dataset, num_workers=n_workers, pin_memory=True,
		batch_sampler=BatchSampler(ImagesSampler(valid_dataset, k, shuffle=False), batch_size=batch_size, drop_last=False))

	test_data = DataLoader(test_dataset, num_workers=n_workers, pin_memory=True,
