...
class ModelPop(nn.Module):
	def __init__(self, n_image_features, vocab_size,
		...
		if self.use_distractors_in_sender:
			...
			sender_image_features = n_image_features
		self.senders = nn.ModuleList([Sender(sender_image_features, vocab_size,
			embedding_dim, hidden_size,
			bound_idx, max_sentence_length, vl_loss_weight, bound_weight, use_gpu) for _ in range(pop_size)])
		self.receivers = nn.ModuleList([Receiver(n_image_features, vocab_size,
			embedding_dim, hidden_size, use_gpu) for _ in range(pop_size)])
		self.shuffle_pair()
